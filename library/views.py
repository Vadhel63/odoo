...
#             send_mail(str(name)+' || '+str(email),message,'EMAIL_HOST_USER', ['omunadkat2@gmail.com'], fail_silently = False)
...
def delete_book(request, raw_id):
    if request.method == 'POST':
        raw = Book.objects.get(id=raw_id)
        raw.delete()
    return redirect("viewbook_view")
